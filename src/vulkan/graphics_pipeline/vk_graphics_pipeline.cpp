...
VkResult VkPipelineClass::init(VkDescriptorClass *pDescriptors) {
    VkResult result;
    if (pDescriptors == nullptr) { return VK_ERROR_DEVICE_LOST; }

    // Link the descriptors and device to the pipeline
    descriptors = pDescriptors;
    device = descriptors->getDevice();

    // Get the compiled shader files
    auto fragShaderCode = readFile(FRAG_SHADER_FILE);
    auto vertShaderCode = readFile(VERT_SHADER_FILE);
    ...
    pipelineLayoutInfo.pSetLayouts = descriptors->getLayout();

    ERROR_RETURN(vkCreatePipelineLayout(device->getDevice(), &pipelineLayoutInfo, nullptr, &pipelineLayout));
    ...
    ERROR_RETURN(vkCreateGraphicsPipelines(device->getDevice(), VK_NULL_HANDLE, 1, &pipelineInfo, nullptr, &graphicsPipeline));
...
std::vector<char> VkPipelineClass::readFile(const std::string& filename) {
